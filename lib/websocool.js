module.exports = new function () {
    ...
    Room.prototype.addParticipant = function (user) {
        ...
        userInRoom.room = this;
        userInRoom.creation = Date.now();
        usersInRooms.push(userInRoom);
    };
    ...
    self.addUserInRoom = function (user_id, room_id) {
	...
	self.getListOfRoomsByUser = function (user_id) {
		var user = users[user_id];
		var res = [];
		...
		for (var key in user.rooms) {
			res.push(user.rooms[key].simplify());
		}	
    ...
    self.getListOfUsersInRoomsFrom = function (from) {
        var res = [];
        var date = Date.now();

        usersInRooms.forEach(function (userInRoom) {
            if (from < userInRoom.creation) {
                res.push({ user: userInRoom.user.simplify(), room: userInRoom.room.simplify() });
            }
        });
